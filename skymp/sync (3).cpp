...
packet_t my_data, other_players[1000];
...
struct mate_t : actor_info
{
	...
	void tick(const player_t &player, ScriptDragon::TESObjectREFR *local_pl_obj)
	{
		...
		//ScriptDragon::PrintNote("%f %f %f id %d", x_dest, y_dest, z_dest, id); return;
		{
			float translate_speed = 1000000;
			...
			static BSFixedString jump("bInJumpState");
			/*if (jumpvar_dest)
			{
				if (!this->isjumping)
				{
			...
			if (jumpvar_dest && shit_dest)
			{
				UInt8 isj;
				((Actor *)this->actor)->animGraphHolder.GetVariableBool(&jump, &isj);
				if (!isj)
				{
					((Actor *)this->actor)->animGraphHolder.SendAnimationEvent(&JumpDirectionalStart);
				}
			...
			else
			{
				...
				{
					translate_speed = boost * ((direction_dest == 0.5f) // S
...
void skyplug::add_player(const player_t &player, size_t id)
{
	...
	ScriptDragon::Actor::StartCombat(new_mate.actor, player.actor /*(ScriptDragon::CActor *)new_mate.dop*/);
